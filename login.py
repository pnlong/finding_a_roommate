...
class instagram_driver:
    ...
    def click_search(self):
        ...
        try: # if seach has opened, not closed
            ...
            self.driver.find_element("xpath", "//div[@aria-label='Clear the search box']").click()
    ...
    def send_message(self, text, scalar = s / 2):
        # type in message
        message_box = self.driver.find_element("xpath", "//div[@aria-describedby='Message']")
        message_box.clear()
        self.simulate_typing(element = message_box, text = text, scalar = scalar)
        del message_box
        self.wait(0.75, 1.5)
        # send message
        self.driver.find_element("xpath", "//div[text()='Send'][@role='button']").click()
        self.wait(1.5, 2.5)
    ...
    def scroll_to_end(self, element, direction, scalar = s): # if direction is negative, scrolls to top; positive, scrolls to bottom
        a0 = None
        a = self.driver.execute_script("return arguments[0].scrollTop;", element)
        while a != a0:
            self.scroll(a = a, b = a + (direction * 100 * m), scalar = scalar, element = element)
            self.wait(0.5, 1)
            a0 = a
            a = self.driver.execute_script("return arguments[0].scrollTop;", element)
        del a0, a
